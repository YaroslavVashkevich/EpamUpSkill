...
public class SqlQueries {
    ...
    //language=HSQLDB
    String select05 = "SELECT e.lastName,e.Salary, d.name  FROM Employee AS e" +
                      " JOIN DEPARTMENT AS d ON e.Department = d.ID";

    //Select all employees and corresponding department name if there is one.
    //Name column containing name of the department "depname".
    //language=HSQLDB
    String select06 = "SELECT  e.lastName, e.Salary, d.name as depname FROM Employee AS e" +
                      " LEFT OUTER JOIN Department AS d ON e.Department = d.ID";
    ...
    //language=HSQLDB
    String select08 = "SELECT  d.name AS depname, COUNT(*) AS staff_size FROM EMPLOYEE AS e " +
                      "JOIN Department AS d ON e.Department = d.ID " +
    ...
    //Name column containing name of the department "depname".
    //language=HSQLDB
    String select09 = "SELECT  d.name as depname, SUM(e.salary) AS total, AVG(e.salary) AS average FROM Employee AS e " +
            "JOIN Department AS d ON e.Department = d.ID WHERE e.department IS NOT NULL " +
            "GROUP BY d.name";
    ...
    String select10 ="SELECT e1.LastName as employee, e2.LastName AS manager " +
                        "FROM Employee AS e1 " +
                        "LEFT JOIN Employee AS e2 ON e1.manager = e2.id";
